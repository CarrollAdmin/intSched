			...
			case _ = <-ticker.C:
				for _, namespace := range Namespaces {
					...
					for it := 0; it < len(Pods.Items); it++ {
						...
						if Pods.Items[it].ObjectMeta.Annotations["app"] == "SLURM-JOB" || Deployment.ObjectMeta.Annotations["dynamic-resource-management"] != "1" || Pods.Items[it].Status.Phase != "Running" {
							continue
						}
						...
						Shellout("kubectl top pods -n slurm " + Pods.Items[it].ObjectMeta.Name + " | grep 'm' | awk '{print $2}' | sed 's/m//' > real_time_cpu ")
