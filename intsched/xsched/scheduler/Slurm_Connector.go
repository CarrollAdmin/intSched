...
func Get_Pods_Slurm_Job_Id(deployment_name string) string {
	...
	Shellout_slurm("squeue --j=" + deployment_name[3:] + " -o\"%.7i %.9P %.8j %.8u %.2t %.10M %.6D %C %N\" | grep -v 'CPUS' > slurm_jobs/pod_job_id" + deployment_name)
...
func Get_Pods_Slurm_Job_Id_Per_Namespace(deployment_name string, namespace string, Virtual_Node_Name string) string {
	...
	Shellout_slurm("./squeue.sh " + Virtual_Node_Name + " " + namespace + " " + deployment_name[3:] + " > slurm_jobs/pod_job_id" + deployment_name + namespace)
