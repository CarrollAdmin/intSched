 ...
 *  OpenSSL. If you modify file(s) wifth this exception, you may extend this
					 ...
					 bool qos_preemptor, bool preempt_mode)
{
	...
	//create slurm resource allocation on the kubernetes scheduler and wait fot the allocated nodelist
	char id[100];
	sprintf(id, " --workload-name=job%d", job_ptr->job_id);
	char command[200] = "slurm_k8s_plugin";
	strcat(command, id);
	sprintf(id, " --node-nums=%d", min_nodes);
	strcat(command, id);
	sprintf(id, " --total-cpus-per-node=%d", job_ptr->details->min_cpus / min_nodes);
	strcat(command, id);
	strcat(command, " >> /tmp/logs");
	// 向/tmp/logs中写入 command，和当前时间
	fp = fopen("/tmp/logs", "a");
	...
	sprintf(id, "%d", job_ptr->job_id);
	char path[80] = "/tmp/slurm_nums/job";
	strcat(path, id);
	printf("path: %s\n", path);
	...
	fp = fopen(path, "r");
	//initialize slurm nodes	TODO: change 1 with the actual slurm's node count
	bit_clear_all(node_bitmap);

	while ((read = getline(&line, &len, fp)) != -1)
	{
		//read the kubernetes node allocation list and initialise the slurm node bitmap
		int node = atoi(line);
		bit_nset(node_bitmap, node, node);
	...
	fprintf(fp, "Time: %s, end select_nodes\n", ctime(&end));
	fclose(fp);
		...
		for (i = i_first, j = 0; i <= i_last; i++)
		{
			...
			{ /* Memory per CPU */
					...
					select_node_record[i].vpus)
				{
					...
					cpu_count *= job_ptr->details->mc_ptr->threads_per_core;
				}
...
top:
	orig_node_map = bit_copy(save_node_map);
	...
	if ((rc != SLURM_SUCCESS) && preemptee_candidates && preempt_by_qos &&
		...
		(job_ptr->priority != 0))
	{ /* Job can be held by bad allocate */
	...
	else if ((rc != SLURM_SUCCESS) && preemptee_candidates)
	{
		...
		while ((tmp_job_ptr = list_next(job_iterator)))
		{
			...
			else
			{
				...
				while ((tmp_job_ptr = list_next(job_iterator)))
				{
					if (tmp_job_ptr->details->usable_nodes == 99999)
						break;
